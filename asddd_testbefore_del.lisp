...
(defun test-throttle-cutoff (throttle-val brake-val) {
    (print (str-from-n throttle-val "Testing throttle cutoff - Throttle: %.2fV"))
    (print (str-from-n brake-val "Testing throttle cutoff - Brake: %.2fV"))
    
    ; Simulate the throttle cutoff logic
    (var test-throttle throttle-val)
    (if (> brake-val min-adc-brake)
        (set 'test-throttle 0)
    )
    
    (print (str-from-n test-throttle "Result - Final throttle: %.2fV"))
    (if (> brake-val min-adc-brake)
        (print "✓ Throttle cutoff active - SAFE")
        (print "✗ Throttle cutoff inactive")
    )
})
...
(defun adc-input(buffer) ; Frame 0x65
    {
        (let ((current-speed (* (get-speed) 3.6))
            ...
            (brake (/(bufget-u8 uart-buf 5) 77.2)))
            {
                ...
                ; Safety: Cut off throttle when brake is applied above threshold
                (if (> brake min-adc-brake)
                    (set 'throttle 0) ; Force throttle to 0 when braking
                )
...
(print "  (test-throttle-cutoff 1.0 0.5) - Test throttle cutoff safety")
